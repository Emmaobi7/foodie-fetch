...
const Cart = () => {
  ...
  const { totalPrice, onRemove, cartItems, totalQuantities, setShowCart, toggleCartItemQuanitity } = useContext(ShopContext);
  

  const handleCheckout = async () => {
    // const stripe = await getStripe();

    const response = await fetch('localhost://5000/payment/checkout', {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="cart-wrapper" to={cartRef}>
              ...
              <div className="flex bottom">
                <div>
                  <p className="quantity-desc">
                    ...
                    <span className="num" onClick="">{totalQuantities}</span>
